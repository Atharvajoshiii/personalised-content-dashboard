...
import { FiBookOpen, FiFilm, FiUsers } from 'react-icons/fi';
...
const CategorizedContentFeed: React.FC = () => {
  ...
  return (
    <div className="w-full p-4 md:p-6">
      ...
      {/* Enhanced tabs with illustrations and better typography */}
      <div className="flex justify-between mb-8 items-center gap-6">
        <Tab.Group selectedIndex={selectedIndex} onChange={setSelectedIndex} className="flex-1">
          <Tab.List 
            className={`flex rounded-2xl p-1.5 shadow-lg border ${
              theme === 'light' 
          ...
          >
            <Tab
              className={({ selected }: { selected: boolean }) =>
                `flex-1 relative overflow-hidden rounded-xl transition-all duration-300 transform hover:scale-[1.02]
                 ${selected
            ...
            >
              <div className="flex flex-col items-center justify-center py-4 px-3 space-y-2">
                <div className={`relative ${selectedIndex === 0 ? 'animate-pulse' : ''}`}>
                  <div className="text-2xl">📰</div>
                  <FiBookOpen className="absolute -bottom-1 -right-1 w-3 h-3 opacity-75" />
                </div>
                <div className="text-center">
                  <div className="font-bold text-sm">News</div>
                  <div className="text-xs opacity-75">({newsItemsOrdered.length} articles)</div>
                </div>
              </div>
            </Tab>
            ...
            >
              <div className="flex flex-col items-center justify-center py-4 px-3 space-y-2">
                <div className={`relative ${selectedIndex === 1 ? 'animate-pulse' : ''}`}>
                  <div className="text-2xl">🎬</div>
                  <FiFilm className="absolute -bottom-1 -right-1 w-3 h-3 opacity-75" />
                </div>
                <div className="text-center">
                  <div className="font-bold text-sm">Movies</div>
                  <div className="text-xs opacity-75">({movieItemsOrdered.length} films)</div>
                </div>
              </div>
            </Tab>
            ...
            >
              <div className="flex flex-col items-center justify-center py-4 px-3 space-y-2">
                <div className={`relative ${selectedIndex === 2 ? 'animate-pulse' : ''}`}>
                  <div className="text-2xl">📱</div>
                  <FiUsers className="absolute -bottom-1 -right-1 w-3 h-3 opacity-75" />
                </div>
                <div className="text-center">
                  <div className="font-bold text-sm">Social</div>
                  <div className="text-xs opacity-75">({socialItemsOrdered.length} posts)</div>
                </div>
              </div>
            </Tab>
            ...
            >
              <div className="flex flex-col items-center justify-center py-4 px-3 space-y-2">
                <div className={`relative ${selectedIndex === 3 ? 'animate-pulse' : ''}`}>
                  <div className="text-2xl">🌟</div>
                  <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full"></div>
                </div>
                <div className="text-center">
                  <div className="font-bold text-sm">All</div>
                  <div className="text-xs opacity-75">({allItems.length} items)</div>
                </div>
              </div>
            </Tab>
          </Tab.List>
  ...
  const renderDragDropLayout = () => (
    <div className="relative">
      ...
      >
        ...
        >
          <div className={`w-full ${
            layout === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10' : 'space-y-10'
          }`}>
            {items.map((item, index) => (
              <SortableContentCard 
                key={item.id} 
